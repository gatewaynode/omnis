...
- Owner's full-window screenshots (2026-09-12, meadow at (16,6) facing west and dungeon at (3,6) facing north): "good progress". One artifact in both: the backdrop shows in the bottom corners beside the party's own floor strip and, in the dungeon, as black wedges at the top and bottom edges. Cause: the cone has only offset 0 at depth 0, so the tiles beside the party are never drawn although their far halves span canvas x 180–240 and 0–60 at distance 1. Follow-up:
  ...
  - [ ] Row width: every row is one tile too narrow at its far end, not only row 0. At distance `z` the canvas shows offsets up to ±0.99·z, so the far edge of row `d` (z = d + 1) shows the tile at offset ±(d + 1): 29 px wide at row 1, 19 px at row 2, 14 px at row 3, tapering to nothing at z ≈ 0.76·(d + 1). Those are the stepped sky wedges at the meadow's edges (the earlier note blamed the band's rectangles; the band shares the same width). Fix: `half = depth + 1` in the cone, `(depth + 1).min(width)` in `slot_positions` (a full kind goes from 38 to 46 slots at detail 6 / width 5), re-bake, re-baseline. About half an hour; the owner decides whether it goes before M3.
