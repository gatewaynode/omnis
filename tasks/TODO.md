...
- [ ] `.mcp.json` written (`${CLAUDE_PROJECT_DIR}/target/debug/omnis-mcp --root … --log …/.omnis/mcp.log`). Still to do, and it needs the owner: `cargo build -p omnis-mcp`, restart Claude Code so it loads the server, then read `.omnis/mcp.log` for the first request (`initialize` = legacy, `_meta` with a protocol version = modern) and record the answer in ARCH §9.2
...
- End to end, with the game window running and the bridge binary driven by hand: `initialize`, `tools/list`, `sim_script` (three commands applied), `game_status` (turn and clock advanced), `map_text`, and `screenshot` returned as PNG image content (25 KB, the canvas). The last step for the "done when" is the owner's: `cargo build -p omnis-mcp`, restart Claude Code so `.mcp.json` loads, and I read `.omnis/mcp.log` to record the era in ARCH §9.2.
