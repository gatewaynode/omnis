...
### 8.2 Pixel pipeline
- Fixed internal resolution 1280×720 for the 4K target (PRD D20): a 960×540 viewport in the top-left, a 320-px right column (minimap at 8 px a tile, location lines, a 96×40-button pad), and a 180-px band below, every region an expression of the inputs in `layout.rs`. Text is a self-authored 5×7 bitmap font in 6×8 cells (`font.rs`, 213×90 cells): the menus paint on an 80×16-cell framed box centred in the viewport (`menu_cell`, so the models' row indices are unchanged), the fight screens on the viewport's 160×67 grid, the band on its 22 rows (`band.rs`: the message line, a roster row per member with group, name, class, level, HP, SP, AC, and condition, the acting member's row barred and marked while the mouse's selection keeps its highlighted name, an eighteen-row event log with the roll math, the help line). The pipeline is Bevy's `pixel_grid_snap` pattern: an inner `Camera2d` rendering to an `Image` target on its own render layer with MSAA off, and an outer camera showing that canvas as a sprite at the largest whole multiple that fits the window's *physical* pixels (`layout::fit`; `cursor::WindowSize` carries the logical size and the scale factor, so a 4K panel driven at 2× logical still shows three physical pixels a canvas pixel; the letterbox is whole pixels, the sprite nudged half a pixel for odd bars). `--window small|medium|large|huge` opens a window of 1280×720, 2560×1440, 3840×2160, or 7680×2160 physical pixels; without it the game is borderless fullscreen on the current monitor (a windowed 3840×2160 is clamped under the menu bar and would fall to 2×). `ImagePlugin::default_nearest()` for all sampling. All game UI is painted Bevy-free into a canvas-sized RGBA raster (`raster.rs`, `widget.rs`, `screen.rs`, `screens.rs`, `panels.rs`, `band.rs`, `combat_screen.rs`) that `UiPlugin` composes into a scratch frame and uploads into a sprite above the viewport only when it changed. It is pixel exact at every scale, appears in canvas captures and the MCP screenshot, and its widgets are hit-tested from the pointer mapped through the letterbox (`cursor.rs`), so the whole UI is testable headless. The tilesets are baked for the viewport's size (`omnis-cli tileset bake`; `texel_scale: 4` keeps the 16-pixel tiles' look at four canvas pixels a texel) and a test holds every loaded tileset to `layout::VIEWPORT_SIZE`.
