...
| | |
|---|---|
| Status | Draft v0.4, under discussion |
| Date | 2026-09-11 |
...
| D20 | Display target | A modern 4K monitor (3840×2160) is the default target for the first UI. The fixed internal resolution is chosen so that 4K is an integer multiple of it and a glyph pixel covers a few real pixels (about 3×3 to 4×4), not the 12×12 of a 320×180 canvas; the 3D viewport keeps its own, coarser pixel size | Keep the 320×180 canvas at 12× on 4K | Owner decision (2026-09-13): the display area is one of the things Omnis can improve over the 1980s originals, and the pixel-art feel is not worth sacrificing a better UI. Cheapest to change while the build is early. 1080p and 1440p stay supported at smaller integer scales. |
...
### 7.3 Combat
- Turn-based. Initiative is an SRD initiative roll. Each combatant takes an action and a bonus action per round; movement is replaced by row position (§8.3).
...
- Actions: attack, cast, use item, dodge, exchange position, run, and SRD class actions.
...
- **Backgrounds and skills**: SRD backgrounds, the eighteen skills, and tool proficiencies. These replace MM2's secondary skills; crawler-specific abilities such as cartography, mountaineering, and pathfinding are expressed as SRD skill or tool proficiencies plus items and spells (§7.2).
...
Each adaptation names the loop problem it solves. An MM2 mechanic that does not solve one is not adopted.
...
| Marching order and engagement | Battle position equals marching order; an engagement marker | The SRD assumes a battle grid, which a first-person tile view lacks. Two rows replace the grid: front-row members can melee and be meleed; back-row members need reach or range. Opportunity attacks and movement rules are dropped; exchange is an action. |
...
| Action economy | Action, bonus action, reaction, movement | One action per round | SRD action and bonus action are kept. Movement is replaced by row position and the exchange action. Reactions are limited to those that do not depend on a grid (shield, counterspell); opportunity attacks are removed. |
...
### 11.1 Technical
- **Engine**: Bevy, latest stable, explicitly exempt from the N-1 rule (D9). Verified 2026-09-11: 0.19.0 released 2026-06-19, 0.19.1 released 2026-08-13, MSRV 1.95.0, itself edition 2024, MIT OR Apache-2.0. Relevant 0.19 facts: resources are now components on singleton entities; text moved to the Parley engine; `bevy_ui` widgets and Feathers gained text and number inputs, dropdowns, list views, and scrollbars, which the editor can use; the `bsn!` macro exists but there is no `.bsn` asset loader yet, so scene files are not a content format for Omnis; the `audio` feature no longer implies `2d` or `ui`. No 0.20 date is announced; the stated cadence is three to four months, so expect one migration around Q4 2026.
- **Language**: Rust, edition 2024, stable toolchain (1.98 installed, MSRV follows Bevy). No nightly.
- **Rendering**: Bevy 2D only. The default display target for the first UI is a modern 4K monitor, 3840×2160 (D20). Target 60 fps at 1080p on integrated graphics from 2020 onward remains the performance floor; pixel-perfect integer scaling from a fixed internal resolution, so 4K, 1440p, 1080p, and 720p each get the largest whole multiple that fits.
...
- **Exact detail depth and internal resolution:** the 320×180 canvas of 2026-09-12 is being raised for the 4K target (D20, 2026-09-13); the exact UI canvas and the 3D viewport's own resolution are set by the display rework plan in `tasks/TODO.md`. Detail depth stays at 4 with the 16×16 placeholder tiles until the final art direction.
