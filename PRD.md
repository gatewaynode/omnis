...
- **Modern presentation (D26):** what modern means for the viewport, the art direction, and who makes the art. Owner direction (2026-09-20): the viewport is a hybrid of 2D and 3D. Which hybrid is not chosen: a later spike builds the same corridor as higher-resolution 2D slots, as 3D geometry with 2D billboards, and as the reverse, and reports frame cost, crate count, art needs, determinism and captures before the choice. Until then the viewport is untouched and D2's "2D rendering" stands as built; smooth rather than integer scaling is decided from the Feathers experiment's report.
